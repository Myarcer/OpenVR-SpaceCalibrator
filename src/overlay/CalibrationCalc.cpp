...
bool CalibrationCalc::SlamFixDriftStep(double dt,
	...
	// --- Time-skew compensation (LINEAR ONLY) ---
	// The ref (SLAM) pose lags the lighthouse target by ~dt_skew (streaming
	// latency). Extrapolate the ref POSITION forward by dt_skew along its
	// finite-difference velocity so sustained walks become real drift signal
	// the EKF can correct DURING movement instead of after it stops.
	// Rotation is deliberately NOT extrapolated: T_meas contains meter-scale
	// translations, so any rotation extrapolation error (velocity EMA lag at
	// bob reversals, SLAM jitter) multiplies into 100s of mm of phantom
	// translation. Rotation lag is handled by k_lever/k_skew_rot R-inflation
	// (filter stays blind during head rotation), which the 2026-07-05 dance
	// session proved must not be weakened.
	...
	if (m_slamFixPrevRefValid && dt > 1e-4 && dt <= 0.1) {
		...
		// Clamp to human-plausible speed - SLAM teleports must not extrapolate.
	...
	ref_comp.linear() = ref_mat.block<3, 3>(0, 0);
